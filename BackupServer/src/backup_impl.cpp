...
bool BackUpImpl::CheckSrcDestDir(
    ...
    case S_IFDIR: // 目录文件
        ...
        while ((dir_entry = readdir(dirp)) != nullptr) {
            if (strcmp(dir_entry->d_name, ".") == 0 ||
                ...
                continue;
            CopyFiles(
                AppendPath(src, dir_entry->d_name),
                dest_file,
                ino_map,
                dir_list,
                origin_src,
...
void BackUpImpl::CopyFileContent(
    const std::string &src,
    const std::string &dest)
{
    // 打开源文件
...
// 时间、权限、拥有者
void BackUpImpl::CopyFileMetadata(
    const std::string &src,
    const std::string &dest)
{
    // 修改时间
...
void BackUpImpl::CopyFileWithMetadata(
    const std::string &src,
    const std::string &dest)
{
...
void BackUpImpl::CopySymlinkWithMetadata(
    const std::string &src,
    const std::string &dest,
    const std::string &origin_src,
    const std::string &origin_dest)
{
    ...
    if (symlink(link_to.c_str(), dest.c_str()) != 0) {
        std::cerr << "Can not symlink " << dest << " to " << link_to
                  << std::endl;
    }
...
std::string
BackUpImpl::AppendPath(const std::string &prefix, const std::string &filename)
{
    if (prefix.back() == '/') {
