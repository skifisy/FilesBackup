    ...
    std::shared_ptr<FileNode> file_node)
{
    ...
    if (target_path.IsRelative()) {
        file_node->meta_.link_to_full_path =
            target_path.ToFullPath().ToString();
    } else {
    ...
    uint64_t &data_offset)
{
    for (auto child : root->children_) {
        ...
        if (!node->children_.empty()) { DumpInitNode(node, data_offset); }
        if (node->meta_.type == static_cast<uint8_t>(FileType::REG)) {
            ...
            if (ret != ino_map_.end()) {
                ...
                ino_map_.emplace(
                    node->meta_.ino, std::make_pair(node, std::string()));
            }
        }
    }
}
...
void FileTree::DumpData(std::ofstream &ofs)
{
    ...
    for (auto node : nodes_to_save_) {
        ...
        if (node->meta_.type == static_cast<uint8_t>(FileType::REG) &&
            ...
            if (!ifs) {
                std::cerr << "无法打开文件: " << node->meta_.origin_path
                          << std::endl;
            }
            while (ifs.read(buffer, BUFFER_SIZE)) {
                ofs.write(buffer, ifs.gcount());
            }
            if (ifs.gcount() > 0) { ofs.write(buffer, ifs.gcount()); }
            data_offset += node->meta_.size;
        }
    }
    ...
    const std::string &target_path)
{
    ...
    case FileType::FLNK:
        ...
        break;
    default:
        std::cerr << "不支持文件'" << pack_node->meta_.pack_path
                  << "'的文件类型" << std::endl;
        break;
    }
...
void FileTree::RecoverRegularFile(
    std::shared_ptr<FileNode> pack_node,
    std::ifstream &ifs,
    const std::string &target_path)
{
...
{
    if (root_->children_.empty()) { return; }
    uint64_t data_offset = header_.file_data_offset;
